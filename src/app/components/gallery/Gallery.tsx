...
const Gallery = () => {
	...
	const shift = (imageDimension + GAP) / 16;

	useEffect(() => {
		if (carouselRef.current) {
			// Calculating how many images can fit in the carousel track
			const length = Math.floor(
				carouselRef.current.offsetWidth / (desiredImageWidth + GAP)
			);

			// If the length is even, we add an extra image to the carousel to keep the number odd so indexed image is always in the center
			if (length % 2 === 0) {
				setImageDimension(
					(carouselRef.current.offsetWidth - GAP * (length + 1)) / (length + 1)
				);
			} else {
				setImageDimension(
					(carouselRef.current.offsetWidth - 16 * (length - 1)) / length
				);
			}
		}
